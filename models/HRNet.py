...
BatchNorm3d_class = nn.BatchNorm3d
BN_MOMENTUM = 0.1
...
class MCF_Block(nn.Module):
    def __init__(self, in_channels, out_channels, batch_normalization=True, kernel_size=3, rate=2):
...
class HRNet(nn.Module):
    ...
    def __init__(self, base=8):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv3d(1, num_channels, kernel_size=3, stride=2, padding=1),
            BatchNorm3d(num_channels, momentum=BN_MOMENTUM),
            nn.ReLU6(inplace=True)
        )
        self.conv1_0 = BasicBlock(num_channels, num_channels, 1)

        self.conv1_1 = nn.Conv3d(num_channels, num_channels, kernel_size=3, stride=2, padding=1,
                                 bias=False)
        self.bn1_1 = BatchNorm3d(num_channels, momentum=BN_MOMENTUM)
        self.relu = nn.ReLU6(inplace=True)
        ...
        self.stage2, pre_stage_channels = self._make_stage(num_inchannels=num_channels, num_branches=2)
        ...
        self.stage3, pre_stage_channels = self._make_stage(num_inchannels=num_channels, num_branches=3)

        self.fuse1 = nn.Conv3d(base, base, 1)
        self.fuse2 = nn.Conv3d(base * 2, base, 1)
        self.fuse3 = nn.Conv3d(base * 3, base, 1)
        self.fuse_layer1 = MCF_Block(base * 3, base, rate=3)
        self.fuse_layer2 = MCF_Block(base * 2, base)
    ...
    def _make_stage(self, num_inchannels, num_branches, multi_scale_output=True):
        num_modules = 2
    ...
    def forward(self, x):
        """
        原始 segmentation 前向，输出体素级 fault 概率图： [B, 1, D, H, W]
        """
        x = self.conv1(x)
        x = self.conv1_0(x)
        x0 = x
        ...
        x3 = self.fuse3(x[2])
        x3 = F.interpolate(x3, size=(w, h, d), mode='trilinear', align_corners=True)
    ...
    def forward_features(self, x):
        """
        编码器前向：截取 segmentation 主干的高层特征，做全局池化得到 [B, out_dim] 向量。
        用于自监督预训练的表征学习。
        """
        x = self.conv1(x)
        x = self.conv1_0(x)
        ...
        # 不再通过 last_layer，而是直接做全局平均池化
        x = F.adaptive_avg_pool3d(x, output_size=(1, 1, 1))  # [B, base,1,1,1]
        x = x.view(x.size(0), -1)                            # [B, base]
        return x


def get_seg_model(cfg, **kwargs):
    model = FaultNet(cfg, **kwargs)
    return model
